"""Audio Engine Service - Luxury product background music generation.

This service generates background music using Replicate's MusicGen model,
creating luxury ambient cinematic audio for product TikTok ads.

PHASE 7 IMPLEMENTATION:
- Simplified to single product-specific music generation method
- Gender-aware prompts (masculine, feminine, unisex)
- Luxury ambient cinematic style for all product ads
...
class AudioEngine:
    ...
    async def generate_product_background_music(
        self,
        duration: float,
        campaign_id: str,
        ...
        """
        Generate background music for luxury product ad (simplified).
        ...
        """
        logger.info(f"🎵 Generating luxury product music ({duration}s, {gender})...")

        try:
            # Create product-specific prompt
            prompt = self._create_product_music_prompt(duration, gender)

            # Generate music via MusicGen
            music_url = await self._call_musicgen_model(prompt, duration)

            # Download and save to local storage
            local_path = await self._save_music_locally(music_url, campaign_id, "luxury_product")

            logger.info(f"✅ Generated product music saved locally: {local_path}")
            return local_path

        except Exception as e:
            logger.error(f"❌ Error generating product music: {e}")
            raise
    ...
    async def generate_background_music(
        ...
        DEPRECATED: Use generate_product_background_music() for product ads.
        Kept for backward compatibility.
    ...
    def _create_product_music_prompt(self, duration: float, gender: str) -> str:
        """Create music prompt for luxury product ad.
        ...
        prompt = (
            f"Luxury ambient cinematic background music for product commercial. "
            f"Mood: {gender_desc}. "
    ...
    def _create_music_prompt(self, mood: str, duration: float, tempo: str) -> str:
        ...
        DEPRECATED: Use _create_product_music_prompt() for product ads.
        Kept for backward compatibility.
        """
        mood_descriptions = {
